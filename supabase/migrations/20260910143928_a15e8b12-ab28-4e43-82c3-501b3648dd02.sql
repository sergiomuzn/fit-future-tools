...
BEGIN
  ...
  -- Solo cuentan como bonos previos los bonos reales del catálogo distintos de prueba.
  SELECT EXISTS(
    SELECT 1
  ...
  IF EXISTS (SELECT 1 FROM public.client_events WHERE client_id = NEW.client_id AND tipo = 'alta') THEN
    RETURN NEW;
  END IF;
